...
<script type="text/javascript">
    ...
    function filterTuVien(arg_keyword){
    	...
    	for(var i =0; i< tr_hastext.length ;i++){
    		var td = tr_hastext.eq(i).find("td:eq(1)");
    		//alert("text: "+ text);
    		if(td.text().toUpperCase().indexOf(keyword) > -1){
    ...
    function filterInput(arg_keyword){
    	...
    	for(var i =0; i< tr_input.length ;i++){
    		var td = tr_input.eq(i).find("td:eq(1)");
    		//alert("text: "+ text);
    		if(td.text().toUpperCase().indexOf(keyword) > -1){
   ...
   $("#xoatuvien").on('show.bs.modal', function(e) {
    ...
    $(".delete-button").click(function(){
        ...
        $(".popup_delete").slideDown('fast');
        $('body').append('<div id="over"></div>');
        $('#over').fadeIn(300);
        $(".popup_delete").find("#delete-record").attr("href",href);
        return false;

    });
    $("#dismiss-delete").click(function(){
         $(".popup_delete").slideUp('fast');
         $('#over').fadeOut(300);
         $('#over').remove();
         return false;
    });
...
<div class="right-content">
        ...
        <div class="popup_delete">
            ...
            <div class="popup_but">
                <a  href="" class="btn btn-primary" id="delete-record">Xóa Bỏ</a>
                <a class="btn btn-success" id="dismiss-delete">Hủy Bỏ</a>
            </div>
        </div>
	...
	<div class="row">
       <!-- <span "><button type="button" class="action-but">Hành Động</button></span>-->
		<p id="filter" class="filter-text"><a class="filter" id="filter1">Lọc Dữ Liệu <i class="fa fa-filter "></i></a></p>
	</div>
	<div class="row">
				...
				<tbody class="title-tbody">
					...
					<tr>
						...
						<td align="center" class="noshow">
                                 <a href="admincaptinh/tuvien/chinhsua/{{$tv->tuvien_id}}" ><i class="fa fa-pencil " style="font-size: 16px"></i>
                                </a>
                               
                            </td>
                            <td align="center" class="noshow">
                                <a  href="admincaptinh/tuvien/xoa/{{$tv->tuvien_id}}"class="delete-button"><i class="fa fa-trash " style="font-size: 16px"></i>
                                </a>
                            </td>
                            <td align="center" class="noshow">
                                <a  href="admincaptinh/tuvien/chitiet/{{$tv->tuvien_id}}"><i class="fa fa-info-circle" style="font-size: 16px"></i>
                                </a>
                            </td>
					</tr>
					@endforeach
